...
export default function App(){
  ...
  async function editarAluno(event) {
    ...
    try {
      await axios.put(`https://api-aluno.vercel.app/aluno/${idSelecionado}`, formData
        // ou
        // {
        // nome:formData.nome,
        // matricula:formData.matricula,
  ...
  async function buscarCursos() {
    ...
    // .get("https://api-aluno.vercel.app/cursos")
    // .then(() =>{
    //   setCursos(response.data.cursos);
    // })
    // .catch(() => {
  ...
  return (
    <div className={styles.container}>
      <h1>Diário Eletrônico {idSelecionado}
          ...
          <tbody>
            {alunos.map((aluno, index) => (
              <tr key={aluno._id}>
                ...
                <td className={styles.center}>{aluno.nome}</td>
                <td className={styles.center}>{aluno.matricula}</td>
                <td className={styles.center}>{aluno.curso}</td>
